...
object GenLogs extends App {
  generate.compile.drain.unsafeRunAsyncAndForget

  def generate: Stream[IO, String] = {
    ...
    val logFile: File = File("src/main/resources/logs.txt").createFileIfNotExists(createParents = true).overwrite("")
    ...
    Stream repeatEval randomLog map log
  }
}
